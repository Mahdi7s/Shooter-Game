    ...
    public class EazySoundManager : MonoBehaviour
    {
        ...
        private static void RemoveNonPersistAudio(Dictionary<int, Audio> audioDict)
        {
            ...
            foreach (int key in keys)
            {
                ...
                {
                    Audio audio = audioDict[key];
                    if (!audio.Persist && audio.Activated && audioPool.ContainsKey(key))
                    {
        ...
        /// <returns>The ID of the created Audio object</returns>
        public static int PlayUiSound(AudioClip clip, float volume, bool loop)
        {
            return PlayAudio(Audio.AudioType.UiSound, clip, volume, loop, false, 0f, 0f, -1f, null);
        }
